...
const RegisterDoctor = ({ toggleRegister }) => {
    ...
    try {
      const response = await fetch('http://localhost:8080/api/doctors/register', {
        ...
        body: JSON.stringify({
          ...
          street: formData.address, // Assuming 'address' is a single field; update as needed
      ...
      if (response.ok) {
        ...
        alert(`Failed to register doctor. Error: ${errorResponse.message || 'Unknown error'}`);
      }
    } catch (error) {
      console.error('Error:', error);
      alert('An error occurred while registering the doctor.');
  ...
  return (
    <div className="registerFormContainer">
      <p className="cancelButton" onClick={toggleRegister}></p>
      ...
      <form onSubmit={handleSubmit} className="doctorRegisterForm">
        ...
        <button type="submit" className="submitButton">Register Doctor</button>
      </form>
    </div>
  );
};
